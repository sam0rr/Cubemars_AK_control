 ...
 */
bool CubemarsAK::isAttached(uint8_t id) const noexcept {
    if (_configs.count(id)) {
        return true;
    }
    Serial.print(F("[AK-LIB] ERROR: Motor ID "));
    Serial.print(id);
 ...
 * @return int32_t Electrical RPM (ERPM). Defaults to 0 if ID not attached.
 */
